.class final Landroid/support/transition/ChangeBounds$9;
.super Landroid/animation/AnimatorListenerAdapter;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/EnclosingMethod;
    value = Landroid/support/transition/ChangeBounds;->a(Landroid/view/ViewGroup;Landroid/support/transition/ai;Landroid/support/transition/ai;)Landroid/animation/Animator;
.end annotation

.annotation system Ldalvik/annotation/InnerClass;
    accessFlags = 0x0
    name = null
.end annotation


# instance fields
.field private mIsCanceled:Z

.field final synthetic pH:Landroid/support/transition/ChangeBounds;

.field final synthetic pK:Landroid/graphics/Rect;

.field final synthetic pL:I

.field final synthetic pM:I

.field final synthetic pN:I

.field final synthetic pO:I

.field final synthetic val$view:Landroid/view/View;


# direct methods
.method constructor <init>(Landroid/support/transition/ChangeBounds;Landroid/view/View;Landroid/graphics/Rect;IIII)V
    .registers 8

    .prologue
    .line 366
    iput-object p1, p0, Landroid/support/transition/ChangeBounds$9;->pH:Landroid/support/transition/ChangeBounds;

    iput-object p2, p0, Landroid/support/transition/ChangeBounds$9;->val$view:Landroid/view/View;

    iput-object p3, p0, Landroid/support/transition/ChangeBounds$9;->pK:Landroid/graphics/Rect;

    iput p4, p0, Landroid/support/transition/ChangeBounds$9;->pL:I

    iput p5, p0, Landroid/support/transition/ChangeBounds$9;->pM:I

    iput p6, p0, Landroid/support/transition/ChangeBounds$9;->pN:I

    iput p7, p0, Landroid/support/transition/ChangeBounds$9;->pO:I

    invoke-direct {p0}, Landroid/animation/AnimatorListenerAdapter;-><init>()V

    return-void
.end method


# virtual methods
.method public final onAnimationCancel(Landroid/animation/Animator;)V
    .registers 3

    .prologue
    .line 371
    const/4 v0, 0x1

    iput-boolean v0, p0, Landroid/support/transition/ChangeBounds$9;->mIsCanceled:Z

    .line 372
    return-void
.end method

.method public final onAnimationEnd(Landroid/animation/Animator;)V
    .registers 7

    .prologue
    .line 376
    iget-boolean v0, p0, Landroid/support/transition/ChangeBounds$9;->mIsCanceled:Z

    if-nez v0, :cond_18

    .line 377
    iget-object v0, p0, Landroid/support/transition/ChangeBounds$9;->val$view:Landroid/view/View;

    iget-object v1, p0, Landroid/support/transition/ChangeBounds$9;->pK:Landroid/graphics/Rect;

    invoke-static {v0, v1}, Landroid/support/v4/view/q;->c(Landroid/view/View;Landroid/graphics/Rect;)V

    .line 378
    iget-object v0, p0, Landroid/support/transition/ChangeBounds$9;->val$view:Landroid/view/View;

    iget v1, p0, Landroid/support/transition/ChangeBounds$9;->pL:I

    iget v2, p0, Landroid/support/transition/ChangeBounds$9;->pM:I

    iget v3, p0, Landroid/support/transition/ChangeBounds$9;->pN:I

    iget v4, p0, Landroid/support/transition/ChangeBounds$9;->pO:I

    invoke-static {v0, v1, v2, v3, v4}, Landroid/support/transition/av;->b(Landroid/view/View;IIII)V

    .line 381
    :cond_18
    return-void
.end method
